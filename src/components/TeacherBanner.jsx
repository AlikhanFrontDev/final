...
export default function TeacherBanner() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const { data: response } = await axios.get(Endpoint + `users/show/teacher/${id}`, { headers: { "Authorization": `Bearer ${token}` } });
        setData(response);
      ...
      <div className="imageBanner">
        ...
        <div className='singleDetail'>
          ...
          <div>
            ...
            <p className='phone'> <AiOutlineUser className='icon' /> {data.subjectName}</p>
          </div>
          <div>
